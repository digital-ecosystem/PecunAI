...
export default function VoiceTermsPhase({ which, onConfirm }: VoiceTermsPhaseProps) {
  ...
  const title    = which === 'terms1' ? "4money" : "Asset Management by froots GmbH";
  const subtitle = which === 'terms1'
  ...
  return (
    ...
    >
      {/* ── Entry transition — phantom orb + card ─────────────────── */}
      <AnimatePresence>
        {showTransition && (
          <>
            <motion.div
      ...
      >
        ...
        >
          ...
          <div className="px-5 pb-5 text-sm" style={{ color: "rgba(15,23,42,0.75)", lineHeight: 1.7 }}>
            {which === 'terms1' ? <FourMoneyInfo /> : <FrootsCustomerInfo />}
          </div>
        </div>
      </motion.div>
